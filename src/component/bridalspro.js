...
import '../App.css';
import { bridalLooks } from './bridals';
...
const BridalInquiryForm = ({ product, selectedSize, onSizeChange }) => {
...
const BridalPro = () => {
  ...
  const product = useMemo(
    () => bridalLooks.find((item) => item.id === productId),
    [productId]
  );
    ...
    return (
      <section className="formal-pro-page">
        <div className="formal-pro-empty">
          <p>The requested bridal design could not be located.</p>
          <button className="formal-form-submit" type="button" onClick={() => navigate('/bridal')}>
            Back to Collection
          </button>
        </div>
      </section>
