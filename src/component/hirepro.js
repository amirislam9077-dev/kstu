...
const HireInquiryForm = ({ product, selectedSize, onSizeChange }) => {
...
const HirePro = () => {
  ...
  const product = useMemo(
    () => hireLooks.find((item) => item.id === productId),
    [productId]
  );
    ...
    return (
      <section className="formal-pro-page">
        <div className="formal-pro-empty">
          ...
          <button className="formal-form-submit" type="button" onClick={() => navigate('/hire')}>
            Back to Collection
          </button>
        </div>
      </section>
