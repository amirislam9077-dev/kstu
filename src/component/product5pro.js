...
import '../App.css';
import { product5Items } from './product5data';
...
const Product5Pro = () => {
  ...
  const product = useMemo(
    () => product5Items.find((item) => item.id === productId),
    [productId]
  );
    ...
    return (
      <section className="formal-pro-page">
        <div className="formal-pro-empty">
          ...
          <button className="formal-form-submit" type="button" onClick={() => navigate('/')}>
            Back to Home
          </button>
        </div>
      </section>
